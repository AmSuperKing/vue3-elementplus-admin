  ...
  <template v-if="item.children && item.children.length">
    <template v-if="item.children.length > 1">
      <el-sub-menu :key="item.path" :index="item.path" class="menu-border-bottom">
        <template #title>
          <CustomIcon v-if="item?.meta?.icon" :name="item?.meta?.icon" color="#fff" style="padding-top: 2px" />
    ...
    <template v-if="item.children.length === 1">
      <router-link :key="item.children[0].path" :to="item.children[0].path">
        <el-menu-item :index="item.children[0].path" class="menu-border-bottom">
          <CustomIcon v-if="item?.meta?.icon" :name="item?.meta?.icon" color="#fff"
...
export default {
  ...
  setup() {
     ...
     * <CustomIcon v-if="item.icon" :name="item.icon" />
     * 具体详细见全局自定义图标组件的使用
     */
  },
}
