<template>
  <div class="container">
    <section class="usage-text">
      ...
      <b><i>svg-</i></b> 为前缀，svg 资源文件存放在 <b><i>'src/assets/svgs'</i></b
      ><br /><br />
      <b>example: </b><br />
...
const getIconList = () => {
  const expandedIconFiles = import.meta.glob('../../assets/svgs/*.svg')

  for (const key in expandedIconFiles) {
    const filename = key.replace(/(\.\.\/\.\.\/assets\/svgs\/|\.svg)/g, '')
