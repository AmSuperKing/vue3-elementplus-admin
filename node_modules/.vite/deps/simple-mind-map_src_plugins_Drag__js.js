import { D as getNodeIndexInNodeList, N as getTopAncestorsFomNodeList, at as throttle, i as bfsWalk, it as sortNodeList, xt as CONSTANTS } from "./utils-x13jKsCZ.js";
import { t as Base } from "./Base-C-GPhs3c.js";
...
var Drag = class extends Base {
	...
	checkOverlapNode() {
		...
		const { LOGICAL_STRUCTURE, LOGICAL_STRUCTURE_LEFT, MIND_MAP, ORGANIZATION_STRUCTURE, CATALOG_ORGANIZATION, TIMELINE, TIMELINE2, VERTICAL_TIMELINE, VERTICAL_TIMELINE2, VERTICAL_TIMELINE3, FISHBONE, FISHBONE2, RIGHT_FISHBONE, RIGHT_FISHBONE2 } = CONSTANTS.LAYOUT;
		this.overlapNode = null;
		this.prevNode = null;
		this.nextNode = null;
	...
	getNewChildNodeDir(node) {
		const { LOGICAL_STRUCTURE, LOGICAL_STRUCTURE_LEFT, MIND_MAP, TIMELINE2, VERTICAL_TIMELINE, VERTICAL_TIMELINE2, VERTICAL_TIMELINE3, FISHBONE, FISHBONE2, RIGHT_FISHBONE, RIGHT_FISHBONE2 } = CONSTANTS.LAYOUT;
		switch (this.mindMap.opt.layout) {
			...
			case FISHBONE2:
			case RIGHT_FISHBONE:
			case RIGHT_FISHBONE2: return node.dir;
			default: return "";
		}
	}
	handleVerticalCheck(node, checkList, isReverse = false) {
		...
		const { VERTICAL_TIMELINE, VERTICAL_TIMELINE2, VERTICAL_TIMELINE3, FISHBONE, FISHBONE2, RIGHT_FISHBONE, RIGHT_FISHBONE2 } = LAYOUT;
		...
		if (isReverse) checkList = checkList.reverse();
	...
	handleHorizontalCheck(node, checkList) {
		...
		if (nodeRect.top <= mouseMoveY && nodeRect.bottom >= mouseMoveY) {
			if (!this.overlapNode && !this.prevNode && !this.nextNode && !node.isRoot) {
				...
				if (checkIsPrevNode) {
					if ([RIGHT_FISHBONE, RIGHT_FISHBONE2].includes(layout)) this.nextNode = node;
					else this.prevNode = node;
					this.setPlaceholderRect({
					...
					if ([RIGHT_FISHBONE, RIGHT_FISHBONE2].includes(layout)) this.prevNode = node;
					else this.nextNode = node;
					this.setPlaceholderRect({
	...
	handleFishbone(node) {
		...
		if (node.layerIndex === 1) this.handleHorizontalCheck(node, checkList);
			...
			if (is2LayerTop || is2MoreLayerBottom) this.handleVerticalCheck(node, checkList, true);
			else this.handleVerticalCheck(node, checkList);
		}
	}
...
//# sourceMappingURL=simple-mind-map_src_plugins_Drag__js.js.map