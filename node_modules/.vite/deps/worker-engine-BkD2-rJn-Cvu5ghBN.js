import { St as pn, U as Wn, _ as It, lt as ft } from "./embedpdf-7TNsu-EA-BqkhwTMZ.js";
import { r as x, t as h } from "./browser-BKLM0ThC-CkSOgtCM-Bk-3vDRi.js";
//#region node_modules/@embedpdf/snippet/dist/worker-engine-BkD2-rJn.js
var i = "RemoteExecutor", s = "Worker", d = class n {
	constructor(e, d) {
		this.worker = e, this.pendingRequests = /* @__PURE__ */ new Map(), this.requestCounter = 0, this.handleMessage = (n) => {
			const e = n.data;
			if ("ready" === e.type) return this.logger.debug(i, s, "Worker is ready"), void this.readyTask.resolve(!0);
			const r = this.pendingRequests.get(e.id);
			if (r) switch (e.type) {
				case "result":
					this.logger.debug(i, s, `Received result for ${e.id}`), r.resolve(e.data), this.pendingRequests.delete(e.id);
					break;
				case "error":
					this.logger.debug(i, s, `Received error for ${e.id}:`, e.error), e.error ? r.fail(e.error) : r.reject({
						code: pn.Unknown,
						message: "Unknown error"
					}), this.pendingRequests.delete(e.id);
					break;
				case "progress": this.logger.debug(i, s, `Received progress for ${e.id}`), r.progress(e.progress);
			}
			else this.logger.warn(i, s, `Received response for unknown request: ${e.id}`);
		}, this.logger = d.logger ?? new ft(), this.worker.addEventListener("message", this.handleMessage), this.readyTask = new It(), this.pendingRequests.set(n.READY_TASK_ID, this.readyTask), this.worker.postMessage({
			id: n.READY_TASK_ID,
			type: "wasmInit",
			wasmUrl: d.wasmUrl,
			logger: d.logger ? Wn(d.logger) : void 0,
			fontFallback: d.fontFallback
		}), this.logger.debug(i, s, "RemoteExecutor created");
	}
	generateId() {
		return `req-${Date.now()}-${this.requestCounter++}`;
	}
	send(n, e) {
		const r = this.generateId(), a = new It(), d = {
			id: r,
			type: "execute",
			method: n,
			args: e
		...
		return this.readyTask.wait(() => {
			this.pendingRequests.set(r, a), this.logger.debug(i, s, `Sending ${n} request:`, r), this.worker.postMessage(d);
		}, (e) => {
			this.logger.error(i, s, `Worker init failed, rejecting ${n}:`, e), a.reject({
				code: pn.Initialization,
	...
	renderPageRaw(n, e, t) {
		return this.send("renderPageRaw", [
			n,
			e,
			t
		]);
	...
	renderPageRect(n, e, t, r) {
		return this.send("renderPageRect", [
			n,
			e,
			t,
			r
	...
	renderThumbnailRaw(n, e, t) {
		return this.send("renderThumbnailRaw", [
			n,
			e,
			t
		]);
	...
	renderPageAnnotationRaw(n, e, t, r) {
		return this.send("renderPageAnnotationRaw", [
			n,
			e,
			t,
			r
	...
	renderPageAnnotationsRaw(n, e, t) {
		return this.send("renderPageAnnotationsRaw", [
			n,
			e,
			t
		]);
	...
	createPageAnnotation(n, e, t, r) {
		return this.send("createPageAnnotation", [
			n,
			e,
			t,
			r
	...
	updatePageAnnotation(n, e, t, r) {
		return this.send("updatePageAnnotation", [
			n,
			e,
			t,
			r
	...
	removePageAnnotation(n, e, t) {
		return this.send("removePageAnnotation", [
			n,
			e,
			t
		]);
	...
	setFormFieldValue(n, e, t, r) {
		return this.send("setFormFieldValue", [
			n,
			e,
			t,
			r
	...
	setFormFieldState(n, e, t, r) {
		return this.send("setFormFieldState", [
			n,
			e,
			t,
			r
	...
	renameWidgetField(n, e, t, r) {
		return this.send("renameWidgetField", [
			n,
			e,
			t,
			r
	...
	shareWidgetField(n, e, t, r, o) {
		return this.send("shareWidgetField", [
			n,
			e,
			t,
			r,
	...
	regenerateWidgetAppearances(n, e, t) {
		return this.send("regenerateWidgetAppearances", [
			n,
			e,
			t
		]);
	...
	redactTextInRects(n, e, t, r) {
		return this.send("redactTextInRects", [
			n,
			e,
			t,
			r
	...
	applyRedaction(n, e, t) {
		return this.send("applyRedaction", [
			n,
			e,
			t
		]);
	...
	flattenAnnotation(n, e, t) {
		return this.send("flattenAnnotation", [
			n,
			e,
			t
		]);
	...
	exportAnnotationAppearanceAsPdf(n, e, t) {
		return this.send("exportAnnotationAppearanceAsPdf", [
			n,
			e,
			t
		]);
	...
	exportAnnotationsAppearanceAsPdf(n, e, t) {
		return this.send("exportAnnotationsAppearanceAsPdf", [
			n,
			e,
			t
		]);
	...
	setDocumentEncryption(n, e, t, r) {
		return this.send("setDocumentEncryption", [
			n,
			e,
			t,
			r
...
d.READY_TASK_ID = "0";
var l = d;
var u = "ImageEncoderPool", c = "Encoder";
var p = class {
	constructor(n = 2, e, t) {
		this.poolSize = n, this.workerUrl = e, this.workers = [], this.pendingTasks = /* @__PURE__ */ new Map(), this.nextWorkerId = 0, this.requestCounter = 0, this.logger = t ?? new ft(), this.initialize();
	}
	initialize() {
		...
		for (let n = 0; n < this.poolSize; n++) try {
			...
			e.onmessage = this.handleWorkerMessage.bind(this), e.onerror = this.handleWorkerError.bind(this), this.workers.push(e), this.logger.debug(u, c, `Worker ${n} created successfully`);
		} catch (e) {
			this.logger.error(u, c, `Failed to create worker ${n}:`, e);
		}
	}
	handleWorkerMessage(n) {
		...
		if (t) if (this.pendingTasks.delete(e.id), "result" === e.type) t.resolve(e.data);
			...
			const n = e.data;
			t.reject(new Error(n.message));
		}
		else this.logger.warn(u, c, `Received response for unknown task: ${e.id}`);
	}
	handleWorkerError(n) {
		this.logger.error(u, c, "Worker error:", n.message);
	}
	getNextWorker() {
	...
	encode(n, e = "image/png", t) {
		return new Promise((r, o) => {
			...
			if (!a) return void o(/* @__PURE__ */ new Error("No workers available in the pool"));
			const i = `encode-${Date.now()}-${this.requestCounter++}`;
			this.pendingTasks.set(i, {
				resolve: r,
				reject: o
			});
			const s = {
			...
			this.logger.debug(u, c, `Sending encoding request ${i} (${n.width}x${n.height})`), a.postMessage(s, [n.data.buffer]);
		});
	}
	destroy() {
		this.logger.debug(u, c, "Destroying worker pool"), this.pendingTasks.forEach((n, e) => {
			n.reject(/* @__PURE__ */ new Error("Worker pool destroyed")), this.logger.debug(u, c, `Rejected pending task: ${e}`);
		}), this.pendingTasks.clear(), this.workers.forEach((n, e) => {
			n.terminate(), this.logger.debug(u, c, `Worker ${e} terminated`);
		}), this.workers = [];
	}
	get activeWorkers() {
