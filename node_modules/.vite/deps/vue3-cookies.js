import { Gn as reactive } from "./vue.runtime.esm-bundler-KfmyCvcZ.js";
//#region node_modules/vue3-cookies/dist/index.js
/**
* Vue3 Cookies v1.0.0
*
* Forked from
...
var VueCookiesManager = function() {
	...
	VueCookiesManager.prototype.config = function(config) {
		for (var propertyName in this.current_default_config) this.current_default_config[propertyName] = config[propertyName] ? config[propertyName] : defaultConfig[propertyName];
	};
	VueCookiesManager.prototype.get = function(keyName) {
		...
		if (value && value.substring(0, 1) === "{" && value.substring(value.length - 1, value.length) === "}") try {
	...
	VueCookiesManager.prototype.set = function(keyName, value, expireTimes, path, domain, secure, sameSite) {
		if (!keyName) throw new Error("Cookie name is not found in the first argument.");
		else if (/^(?:expires|max-age|path|domain|secure|SameSite)$/i.test(keyName)) throw new Error("Cookie name illegality. Cannot be set to [\"expires\",\"max-age\",\"path\",\"domain\",\"secure\",\"SameSite\"]	 current key name: " + keyName);
		if (value && value.constructor === Object) value = JSON.stringify(value);
		var _expires = "";
		if (expireTimes == void 0) expireTimes = this.current_default_config.expireTimes ? this.current_default_config.expireTimes : "";
		if (expireTimes && expireTimes != 0) switch (expireTimes.constructor) {
			case Number:
			...
			case String:
				...
				} else _expires = "; expires=" + expireTimes;
				break;
			case Date:
	...
	VueCookiesManager.prototype.remove = function(keyName, path, domain) {
		if (!keyName || !this.isKey(keyName)) return false;
	...
	VueCookiesManager.prototype.keys = function() {
		...
		for (var _index = 0; _index < _keys.length; _index++) _keys[_index] = decodeURIComponent(_keys[_index]);
		return _keys;
	};
	return VueCookiesManager;
}();
var dist_default = { install: function(app, options) {
	app.config.globalProperties.$cookies = new VueCookiesManager();
	if (options) app.config.globalProperties.$cookies.config(options);
} };
var GLOBAL_COOKIES_MANAGER = null;
function globalCookiesConfig(options) {
	if (GLOBAL_COOKIES_MANAGER == null) GLOBAL_COOKIES_MANAGER = new VueCookiesManager();
	GLOBAL_COOKIES_MANAGER.config(options);
}
function useCookies() {
	if (GLOBAL_COOKIES_MANAGER == null) GLOBAL_COOKIES_MANAGER = new VueCookiesManager();
	return { cookies: reactive(GLOBAL_COOKIES_MANAGER) };
}
//#endregion
export { dist_default as default, globalCookiesConfig, useCookies };
